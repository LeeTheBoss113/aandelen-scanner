...
def scan_aandeel(ticker, sector):
    try:
        # We halen 2 jaar aan data op om de SMA 252 (jaarlijks) goed te berekenen
        ...
        hi = float(close.tail(252).max()) # Hoogste punt van het afgelopen jaar
        dist_top = ((hi - curr) / hi) * 100
        
        # --- SCORE BEREKENING (AANGEPAST) ---
        # We geven een bonus als de koers boven de gemiddeldes zit (Trend is je Friend)
        score = (100 - float(rsi)) + (dist_top * 1.5)
        if is_above_1y: score += 10  # Bonus voor gezonde lange termijn trend
        if is_above_3m: score += 5   # Bonus voor positief kwartaal momentum
...
# --- 5. VISUALISATIE (Met nieuwe kolommen) ---
if all_results:
